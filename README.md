...
concepts:
  ...
  - Modrinth mod and plugin management
  - World management with dimension grouping
  - Automated backups (all dimensions) and scheduling
...
- Список установленных модов (с client/server side меткой)
- Удаление модов
- Проверка обновлений
- Навигация «назад» сохраняет позицию в результатах поиска

### Плагины (Modrinth)
- Поиск и установка серверных плагинов для Paper/Purpur/Spigot
- Отдельный раздел «Плагины» в меню (только для plugin loaders)
- Управление аналогично модам: поиск, установка, удаление

### Бэкапы
- Ручное и автоматическое создание бэкапов
- **Группировка измерений**: бэкап включает overworld + nether + the_end в один архив
...
- Восстановление из бэкапа (все измерения)
- Клонирование бэкапа в новый мир (с переименованием измерений)
- Ротация старых бэкапов

### Миры
- Список миров с суммарными размерами (overworld + nether + the_end)
- **Группировка измерений**: `world`, `world_nether`, `world_the_end` отображаются как один мир
...
- Переименование миров (с автообновлением `server.properties`, все измерения)
- Клонирование миров (все измерения)
- Удаление неактивных миров с подтверждением (все измерения)
- Бэкап конкретного мира из меню миров

### Конфигурация
- Просмотр и редактирование `server.properties` через бота
- **YAML конфиги**: Paper, Pufferfish, Spigot, Bukkit (чтение/запись через PyYAML)
- Категории: Производительность, Геймплей, Мир, Сеть, Paper оптимизация, Paper сущности, Pufferfish, Spigot
- Кнопочный выбор значений для каждого типа свойства:
  - **bool** → ВКЛ / ВЫКЛ
  - **enum** → кнопки с вариантами (difficulty, gamemode, redstone-implementation)
  - **range** → пресеты + ручной ввод (view-distance, max-players)
  - **text** → FSM-ввод (motd, level-name, seed)
- **Шаблон «Оптимизация»**: применяет 17 оптимальных настроек одной кнопкой
- Валидация по движку: Paper/Purpur настройки пропускаются на Forge/Fabric
- Информация о сервере: версия MC, тип, RAM (read-only из Docker)
- Aikar's JVM G1GC flags для оптимизации сборки мусора
...
- Топ игроков по playtime с пагинацией и онлайн-индикатором
- Фильтрация по периодам: сегодня, 7 дней, 30 дней, всё время
- Графики активности по часам и дням (текстовые гистограммы)
- Детальная статистика по игроку: сессии, средняя длительность, графики
- Лог сессий (join/leave с длительностью)
- **Часовые пояса**: авто-определение из Telegram (русский → MSK), ручной выбор из 14 пресетов
- Все даты и графики отображаются в локальном времени пользователя
...
└── bot/
    ...
    │                           #   installed_mods, installed_plugins, backups,
    │                           #   scheduled_tasks, player_sessions,
    │                           #   bot_settings, user_settings
    │
    ├── minecraft/
    ...
    │   ├── plugin_manager.py   # Установка/удаление плагинов (Paper/Purpur)
    │   ├── server_config.py    # server.properties + YAML конфиги (Paper/Pufferfish/Spigot/Bukkit)
    │   └── world_manager.py    # Миры с группировкой измерений (_nether, _the_end)
    │
    ├── services/
    ...
    │   ├── plugins.py          # Плагины: поиск/установка/удаление (Paper/Purpur)
    │   ├── backups.py          # Бэкапы: создание/список/скачивание/восстановление (все измерения)
    │   ├── config_editor.py    # server.properties + YAML: категории, кнопочный выбор
...
MC_MEMORY=6G                 # Память для JVM сервера
```
...
| YAML Config | [PyYAML](https://pyyaml.org/) | 6.0 |
...
| Миры = директории с level.dat + группировка _nether/_the_end | Paper/Purpur выносят измерения в отдельные папки; все операции (бэкап, клон, удаление) работают со всеми |
| YAML конфиги через PyYAML | Paper/Pufferfish/Spigot хранят настройки в .yml файлах; dot-separated пути для вложенных ключей |
| Короткие ключи свойств (p:opt-expl) | Telegram callback_data ограничена 64 байтами; сокращённые имена для YAML свойств |
| Per-user timezone в user_settings | Каждый пользователь видит статистику в своём часовом поясе; авто-определение из language_code |
...
flowchart TD
    ...
    C --> D["sleep(3)"]
    D --> E["tar.gz world + world_nether + world_the_end"]
    E --> F["RCON: save-on"]
    F --> G["Запись в SQLite"]
    G --> H{"Автоматический?"}
