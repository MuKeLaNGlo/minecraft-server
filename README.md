...
### Планировщик
- Автоматические бэкапы по cron-расписанию
...
- **FSM-ввод**: новые сообщения появляются только когда нужен текстовый ввод (ник, команда, cron)
...
└── bot/
    ...
    │
    ├── states/
    │   └── states.py           # FSM StatesGroups (11 групп, вкл. WorldState, RconState, ServerState)
    │
    ├── filters/
...
stateDiagram-v2
    ...
    state "Scheduler" as Scheduler {
        selecting_type --> waiting_cron
        waiting_cron --> waiting_extra
        waiting_extra --> confirm_delete
    }
